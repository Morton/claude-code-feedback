...
(() => {
  ...
  var require_html2canvas = __commonJS({
    "node_modules/.pnpm/html2canvas@1.4.1/node_modules/html2canvas/dist/html2canvas.js"(exports, module) {
      (function(global, factory) {
        ...
        var createAdditiveCounter = function(value, min, max, symbols, fallback2, suffix) {
  ...
  function resolveAnchor(clientX, clientY) {
    const node = document.elementsFromPoint(clientX, clientY).find((e) => e instanceof HTMLElement && !isOwnUi(e));
    if (!node) {
      return { selector: "", elementTag: "" };
    }
    let selector2 = "";
    try {
      selector2 = finder(node);
    } catch {
      selector2 = "";
    }
...
${nested.join("\n")}
  ...
  function trackStatus(id, pin) {
    ...
    const timer = window.setInterval(async () => {
      ...
      try {
        ...
        if (res.ok) {
          status = (await res.json())?.status ?? null;
          misses = 0;
        } else {
          misses += 1;
        }
  ...
  function injectStyles() {
    ...
    style.textContent = `@keyframes ccf-pulse {0% { box-shadow: 0 0 0 0 rgba(${ACCENT_RGB},0.55); }70% { box-shadow: 0 0 0 12px rgba(${ACCENT_RGB},0); }100% { box-shadow: 0 0 0 0 rgba(${ACCENT_RGB},0); }}`;
